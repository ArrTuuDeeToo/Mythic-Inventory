...
function getInventory(src, Owner, Type, limit)
	if LoadedEntitys[tonumber(Type)].shop then
		...
		local result = MySQL.query.await('SELECT id, count(id) as Count, name as Owner, item_id as Name, dropped as Temp, MAX(quality) as Quality, information as MetaData, slot as Slot, MIN(creationDate) AS CreateDate FROM inventory WHERE name = ? GROUP BY slot ORDER BY slot ASC', {
			queryName
		}) or {}
		return result
	end
	...
	CreateThread(function()
		...
		if data.invTypeFrom == 3 or data.invTypeTo == 3 then
			...
			if data.isSplit then
				...
				-- Normal move işlemi
				if slotTo == nil then
					local result = MySQL.query.await('UPDATE inventory SET slot = ?, name = ?, dropped = ? WHERE name = ? AND slot = ?', {
						data.slotTo,
						string.format("%s-%s", data.ownerTo, data.invTypeTo),
						data.invTypeTo == 10 and 1 or 0,
						string.format("%s-%s", data.ownerFrom, data.invTypeFrom),
					...
					local result1 = MySQL.query.await('UPDATE inventory SET slot = ?, name = ?, dropped = ? WHERE name = ? AND slot = ?', {
						data.slotTo,
						string.format("%s-%s", data.ownerTo, data.invTypeTo),
						data.invTypeTo == 10 and 1 or 0,
						string.format("%s-%s", data.ownerFrom, data.invTypeFrom),
					...
					local result2 = MySQL.query.await('UPDATE inventory SET slot = ?, name = ?, dropped = ? WHERE name = ? AND slot = ?', {
						data.slotFrom,
						string.format("%s-%s", data.ownerFrom, data.invTypeFrom),
						data.invTypeFrom == 10 and 1 or 0,
						string.format("%s-%s", data.ownerTo, data.invTypeTo),
			...
			if not isRecurse then
				...
				if plyr then
					...
					if source then
						...
						Logger:Error("mythic-inventory", "AddItem - plyr:GetData('Source') is nil for Owner: " .. tostring(Owner))
					end
				else
					Logger:Error("mythic-inventory", "AddItem - Fetch:SID(Owner) is nil for Owner: " .. tostring(Owner))
				end
			end
		end
...
RegisterNetEvent("Inventory:Server:GetDropzoneItems", function(dropzoneId)
	...
	for i, item in ipairs(rawResult) do
		print("  " .. i .. ": " .. (item.item_id or "nil") .. " (Count: " .. (item.count or "nil") .. ", Slot: " .. (item.slot or "nil") .. ")")
	end
...
RegisterNetEvent("Inventory:Server:ProcessCartPayment", function(data)
	...
	local char = player:GetData('Character')
	if not char then 
		Logger:Info("mythic-inventory", "Server: ProcessCartPayment - Character not found")
		return 
	end
	...
	Logger:Info("mythic-inventory", "Server: ProcessCartPayment - Payment type: " .. paymentType .. ", Cart items: " .. #cart .. ", Shop ID: " .. tostring(shopId))
	
	if not cart or #cart == 0 then
		Logger:Info("mythic-inventory", "Server: ProcessCartPayment - Cart is empty")
		return
	end
	...
	if not shop then
		Logger:Info("mythic-inventory", "Server: ProcessCartPayment - Shop not found for ID: " .. tostring(shopId))
		TriggerClientEvent("mythic-notify:client:SendAlert", source, { type = "error", text = "Shop not found!" })
		return
	end
	
	Logger:Info("mythic-inventory", "Server: ProcessCartPayment - Shop found: " .. shop.name)
	...
	for _, cartItem in ipairs(cart) do
		...
		if item then
			...
			Logger:Info("mythic-inventory", "Server: ProcessCartPayment - Item: " .. cartItem.name .. ", Price: " .. item.price .. ", Quantity: " .. cartItem.quantity)
		else
			Logger:Info("mythic-inventory", "Server: ProcessCartPayment - Item not found in database: " .. cartItem.name)
		end
	end
	
	Logger:Info("mythic-inventory", "Server: ProcessCartPayment - Total cost: $" .. totalCost)
	...
	Logger:Info("mythic-inventory", "Server: ProcessCartPayment - Cash: $" .. cash .. ", Selected payment type: " .. tostring(data.paymentType))
	
	if not canPay then
		Logger:Info("mythic-inventory", "Server: ProcessCartPayment - Insufficient funds")
	...
	if data.paymentType == 'cash' then
		...
		Logger:Info("mythic-inventory", "Server: ProcessCartPayment - Cash payment result: " .. tostring(paid))
	elseif data.paymentType == 'bank' then
		...
		Logger:Info("mythic-inventory", "Server: ProcessCartPayment - Bank payment result: " .. tostring(paid))
		if paid then
			-- Phone notification'ı güvenli şekilde çağır
			local success, error = pcall(function()
				Phone.Notification:Add(source, "Bill Payment Successful", "Shopping Cart Purchase", os.time() * 1000, 3000, "bank", {})
			end)
			if not success then
				Logger:Info("mythic-inventory", "Server: ProcessCartPayment - Phone notification error: " .. tostring(error))
			end
		end
	end
	
	if not paid then
		Logger:Info("mythic-inventory", "Server: ProcessCartPayment - Payment failed")
		TriggerClientEvent("mythic-notify:client:SendAlert", source, { 
			type = "error", 
			text = "Payment failed!" 
		})
		...
		if item then
			...
			then
				...
				if success and insData then
					Logger:Info("mythic-inventory", "Server: ProcessCartPayment - Created item: " .. cartItem.name .. " x" .. cartItem.quantity)
					-- Store log (mevcut sistem gibi)
					CreateStoreLog(string.format("shop:%s", shopId), cartItem.name, cartItem.quantity, char:GetData("SID"), insData.metadata, insData.id)
				else
					Logger:Info("mythic-inventory", "Server: ProcessCartPayment - CreateItem error: " .. tostring(insData))
				end
			else
				Logger:Info("mythic-inventory", "Server: ProcessCartPayment - Item not eligible: " .. cartItem.name)
			end
		end
	end
	
	-- Başarı mesajı
	Logger:Info("mythic-inventory", "Server: ProcessCartPayment - Sending notification to source: " .. tostring(source))
	...
	Logger:Info("mythic-inventory", "Server: ProcessCartPayment - Payment completed successfully")
end)